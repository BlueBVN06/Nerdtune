...
public class DuelCommand {
    ...
    public void duel(@Sender Player player, Player target) {
        ...
        if (userProfile.getState().equals(ProfileState.IN_PARTY) && !userProfile.getGameData().getParty().isLeader(player.getUniqueId()) && targetProfile.getState().equals(ProfileState.IN_PARTY) && targetProfile.getGameData().getParty().isLeader(target.getUniqueId())) {
            player.sendMessage(CC.error("You can't duel with leader!"));
            return;
        }
